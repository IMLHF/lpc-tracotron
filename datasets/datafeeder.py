...
class DataFeeder(threading.Thread):
    ...
    def __init__(self, coordinator, metadata_filename, hparams):
        ...
        with open(metadata_filename, encoding='utf-8') as f:
            ...
            hours = sum((int(x[1]) for x in self._metadata)) * \
        ...
        self._placeholders = [
            ...
            tf.placeholder(
                tf.float32, [None, None, hparams.num_lpcs], 'lpc_targets'),
            tf.placeholder(tf.float32, [None, None], 'stop_token_targets')
        ]

        # Create queue for buffering data:
        queue = tf.FIFOQueue(
            8, [tf.int32, tf.int32, tf.float32, tf.float32], name='input_queue')
        self._enqueue_op = queue.enqueue(self._placeholders)
        self.inputs, self.input_lengths, self.lpc_targets, self.stop_token_targets = queue.dequeue()
        self.inputs.set_shape(self._placeholders[0].shape)
        self.input_lengths.set_shape(self._placeholders[1].shape)
        self.lpc_targets.set_shape(self._placeholders[2].shape)
    ...
    def _get_next_example(self):
        ...
        input_data = np.asarray(text_to_sequence(
            text, self._cleaner_names), dtype=np.int32)
        lpc_target = np.load(os.path.join(self._datadir, meta[0]))
        stop_token_target = np.asarray([0.] * len(lpc_target))
        return (input_data, lpc_target, stop_token_target, len(lpc_target))
...
def _prepare_batch(batch, outputs_per_step):
    ...
    lpc_targets = _prepare_targets([x[1] for x in batch], outputs_per_step)
    stop_token_targets = _prepare_stop_token_targets(
        [x[2] for x in batch], outputs_per_step)
    return (inputs, input_lengths, lpc_targets, stop_token_targets)
